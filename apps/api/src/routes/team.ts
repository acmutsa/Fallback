...
const teamHandler = HonoBetterAuth()
	...
	.post("/join", zValidator("query", joinTeamSchema), async (c) => {
		...
		await tx.update(teamInvite).set({
			acceptedAt:compDate
	...
	.get("/:teamId",zValidator("param", teamIdValidator), async (c)=>{
		...
		const canUserView = await isUserSiteAdminOrQueryHasPermissions(user, asyncCallback);
		

		if (!canUserView){
			return c.json({message:API_ERROR_MESSAGES.invalidPermissions}, 401)
		}
	...
	.get("/:teamId/admin", zValidator("param", teamIdValidator), async (c)=>{
		...
		const canUserView = isUserSiteAdminOrQueryHasPermissions(user,getAdminUserForTeam(user.id, teamId));
	...
	.delete("/:teamId", zValidator("param", teamIdValidator), async (c)=>{
		...
		const canUserDelete = await isUserSiteAdminOrQueryHasPermissions(user,getAdminUserForTeam(user.id, teamId));
	...
	.get("/:teamId/members", zValidator("param", teamIdValidator), async (c)=>{
		const teamId = c.req.param("teamId");
		const user = c.get("user");
		...
		const canUserView = await isUserSiteAdminOrQueryHasPermissions(user,getAdminUserForTeam(user.id, teamId));
	...
	.patch("/:teamId/update", zValidator("param", teamIdValidator), zValidator("json",teamNameValidator), async (c)=>{
		const user = c.get("user");
		const teamId = c.req.param("teamId");
		const newTeamName = c.req.valid("json");

		if (!user){
			return c.json({message:API_ERROR_MESSAGES.notAuthorized}, 401);
		}

		const canUserUpdate = await isUserSiteAdminOrQueryHasPermissions(user,getAdminUserForTeam(user.id, teamId));
	...
	.delete("/:teamId/:userId/remove", zValidator("param", userTeamActionSchema), async (c)=>{
		const teamId = c.req.param("teamId");
		const userIdToRemove = c.req.param("userId");
		const user = c.get("user");

		if (!user) {
			return c.json({ messsage: API_ERROR_MESSAGES.notAuthorized }, 401);
		}
		...
		if (userIdToRemove === user.id){
			...
			return c.json({ message: "Successfully removed from team." }, 200);
		}
		...
		const canUserRemove = await isUserSiteAdminOrQueryHasPermissions(user,getAdminUserForTeam(user.id, teamId));
